...
const loadConfig = async () => {
  try {
    ...
    if (config.flowDir) config.flowDir = resolveTilde(config.flowDir);
    if (config.flowCommandDir) config.flowCommandDir = resolveTilde(config.flowCommandDir);
    return config;
  } catch (error) {
...
const loadFlows = async () => {
  try {
    ...
    return flows.map((f) => ({
      ...f,
      path: f.path ? resolveTilde(f.path) : f.path,
      script: f.script ? resolveTilde(f.script) : f.script,
    }));
  } catch (error) {
